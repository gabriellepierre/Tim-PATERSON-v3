 ...
 */
final class Version20220130215254 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('DROP TABLE gabrielle');
        $this->addSql('DROP TABLE tiff');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TABLE gabrielle (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(180) NOT NULL COLLATE BINARY, roles CLOB NOT NULL COLLATE BINARY --(DC2Type:json)
        , password VARCHAR(255) NOT NULL COLLATE BINARY)');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_B50E38D5F85E0677 ON gabrielle (username)');
        $this->addSql('CREATE TABLE tiff (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(180) NOT NULL COLLATE BINARY, roles CLOB NOT NULL COLLATE BINARY --(DC2Type:json)
        , password VARCHAR(255) NOT NULL COLLATE BINARY)');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_15691034F85E0677 ON tiff (username)');
        $this->addSql('DROP INDEX IDX_23A0E6612469DE2');
        $this->addSql('CREATE TEMPORARY TABLE __temp__article AS SELECT id, category_id, title, content, auteur, date, tags, etat, resume FROM article');
        $this->addSql('DROP TABLE article');
